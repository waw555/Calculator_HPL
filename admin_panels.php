...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['create_panel', 'update_panel'])) {
        ...
        $decorColor = trim($_POST['decor_color'] ?? '');
        ...
        if ($decorColor !== '' && !preg_match('/^#[0-9a-fA-F]{6}$/', $decorColor)) $errors[] = 'Цвет декора должен быть указан в формате #RRGGBB.';
    ...
    <section class="panel">
        ...
        <form method="post" enctype="multipart/form-data" id="panel-form">
            ...
            <div class="grid-2">
                ...
                <div><label>Цвет декора для 3D-модели</label><input id="decor_color" type="color" name="decor_color" value="<?php echo e((string)($editing['decor_color'] ?? '#d9dde3')); ?>"><div class="hint">Используется в визуализации перегородки.</div></div>
