...
let currentCurrencyRate=parseFloat(currencyInput?.selectedOptions[0]?.dataset.rate||'1')||1,lastPriceSource='m2';
...
currencyInput?.addEventListener('change',()=>{const newRate=parseFloat(currencyInput.selectedOptions[0]?.dataset.rate||'1')||1;const mult=currentCurrencyRate/newRate;[priceM2Input,priceSheetInput,costInput].forEach(inp=>{const v=parseFloat(inp.value);if(Number.isFinite(v))inp.value=fmt(v*mult);});currentCurrencyRate=newRate;});
window.addEventListener('appcurrencychange',event=>{const code=event.detail?.code;if(!currencyInput||!code||currencyInput.value===code)return;const option=Array.from(currencyInput.options).find(opt=>opt.value===code);if(!option)return;const oldRate=currentCurrencyRate,newRate=parseFloat(option.dataset.rate||'1')||1,mult=oldRate/newRate;[priceM2Input,priceSheetInput,costInput,costSheetInput].forEach(inp=>{const v=parseFloat(inp?.value);if(Number.isFinite(v))inp.value=fmt(v*mult);});currencyInput.value=code;currentCurrencyRate=newRate;});
