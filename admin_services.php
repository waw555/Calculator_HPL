    ...
    <section class="panel">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="grid">
                ...
                <div><label>Валюта</label><select id="currency" name="currency"><?php foreach ($currencies as $c): ?><option value="<?php echo e($c['code']); ?>" data-rate="<?php echo e((string)$c['rate_to_rub']); ?>" <?php echo (string)($editing['currency']??'RUB')===$c['code']?'selected':''; ?>><?php echo e(app_currency_symbol((string)$c['code'])); ?> — <?php echo e($c['name']); ?></option><?php endforeach; ?></select></div>
    ...
    <section class="panel">
        ...
        <?php foreach ($grouped as $group): ?>
            ...
            <tr>
                ...
                <td class="price"><?php echo e(number_format((float)$svc['price'],2,',',' ')); ?> <?php echo e(app_currency_symbol((string)($svc['currency']??'RUB'))); ?></td>
            ...
            <div><label>Услуги в комплекте</label>
                <div class="services-list" id="services-list">
                    ...
                    <?php $hLabels=['no'=>'','le_2_5'=>'≤ 2.5','2_5_to_5'=>'2.5–5','le_3'=>'≤ 3','3_to_6'=>'3–6'];$dLabels=['no'=>'','le_4'=>'≤ 4','4_to_12'=>'4–12','gt_12'=>'> 12'];$stepLabels=['no'=>'','16'=>'16','32'=>'32','64'=>'64'];$grouped2=[];foreach($allServices as $svc){$key=($svc['category_id']??null)?(string)$svc['category_id']:'_none';if(!isset($grouped2[$key]))$grouped2[$key]=['name'=>$svc['category_name']?:'Без категории','items'=>[]];$grouped2[$key]['items'][]=$svc;} foreach($grouped2 as $group):?>
                        ...
                        <label><input type="checkbox" name="service_ids[]" value="<?php echo (int)$svc['id']; ?>" <?php echo in_array((int)$svc['id'],$selectedServices)?'checked':''; ?>><span><?php echo e((string)($svc['nomenclature']??'')); ?> — <?php echo e($svc['name']); ?><?php if($tags):?>, <?php echo e(implode(', ',$tags));?><?php endif;?></span><span class="service-meta"><?php echo e(number_format((float)$svc['price'],2,',',' ')); ?> <?php echo e(app_currency_symbol((string)($svc['currency']??'RUB'))); ?> / <?php echo e($svc['unit']); ?></span></label>
