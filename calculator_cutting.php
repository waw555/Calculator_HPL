...
<main class="container">
    ...
    <section class="panel">
        <div class="parts-header"><div class="section-title main">Детали для раскроя</div><button type="button" class="add-part-open" id="show-part-dialog">＋ Добавить деталь</button></div>
        ...
        <div id="part-dialog" class="entry-dialog hidden" role="dialog" aria-modal="true" aria-labelledby="part-dialog-title"><div class="entry-dialog__box"><div class="entry-dialog__header"><span class="entry-dialog__icon">▰</span><div class="entry-dialog__title"><strong id="part-dialog-title">Добавить деталь</strong><small>Задайте размеры, количество и возможность поворота</small></div><button type="button" class="entry-dialog__close part-dialog-close" aria-label="Закрыть">×</button></div><div class="entry-dialog__grid"><div class="wide"><label for="new-part-name">Наименование</label><input id="new-part-name" type="text" placeholder="Например, боковина"></div><div><label for="new-part-length">Длина, мм</label><input id="new-part-length" type="number" min="1" placeholder="Длина"></div><div><label for="new-part-width">Ширина, мм</label><input id="new-part-width" type="number" min="1" placeholder="Ширина"></div><div><label for="new-part-qty">Количество</label><input id="new-part-qty" type="number" min="1" value="1"></div><div><label for="new-part-rotate">Поворот</label><label style="display:flex;align-items:center;gap:8px;margin-top:10px"><input id="new-part-rotate" type="checkbox" checked style="width:auto"> Разрешить поворот на 90°</label></div></div><div class="entry-dialog__actions"><button type="button" class="secondary part-dialog-close">Отмена</button><button type="button" id="add-part-btn">Добавить деталь</button></div></div></div>
...
function renderPanelSearch() {
    ...
    const rows = matches.map((item, index) => `<tr><td class="library-list__manufacturer">${escapeHtml(item.manufacturer_name || '—')}<span class="library-kind ${item.type}">${item.type === 'format' ? 'формат' : 'декор'}</span></td><td class="library-list__format">${Number(item.height_mm)} × ${Number(item.width_mm)} мм</td><td class="library-list__decor-number">${escapeHtml(item.decor_number || '—')}</td><td>${escapeHtml(item.decor_name || (item.type === 'decor' ? item.name : '') || '—')}</td><td class="library-list__action"><button type="button" data-library-index="${index}">＋ Добавить</button></td></tr>`).join('');
...
function renderParts() {
    ...
    if (!parts.length) partsTbody.innerHTML = '<tr class="parts-empty"><td colspan="7">Детали пока не добавлены. Нажмите «Добавить деталь».</td></tr>';
...
const partDialog=document.getElementById('part-dialog');
function showPartDialog(show=true){partDialog.classList.toggle('hidden',!show);document.body.style.overflow=show?'hidden':'';if(show)requestAnimationFrame(()=>document.getElementById('new-part-name').focus());}
document.getElementById('show-part-dialog').addEventListener('click',()=>showPartDialog());
document.querySelectorAll('.part-dialog-close').forEach(button=>button.addEventListener('click',()=>showPartDialog(false)));
partDialog.addEventListener('click',event=>{if(event.target===partDialog)showPartDialog(false);});
document.getElementById('add-part-btn').addEventListener('click',()=>{
    ...
    name.value='';length.value='';width.value='';qty.value=1;renderParts();showPartDialog(false);
});
[document.getElementById('new-part-name'),document.getElementById('new-part-length'),document.getElementById('new-part-width'),document.getElementById('new-part-qty')].forEach(input=>input.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();document.getElementById('add-part-btn').click();}}));
renderParts();
