...
.cost-card{display:flex;flex-direction:column;padding:28px 26px;background:#111a2d;color:#fff;border:0!important}.cost-card__heading{display:flex;align-items:baseline;justify-content:space-between;gap:18px}.cost-card__eyebrow{color:#ff4f78;font-size:15px;font-weight:900;text-transform:uppercase}.cost-card__total{flex:none;font-size:18px;font-weight:900;text-align:right;white-space:nowrap}.cost-card__section+.cost-card__section{margin-top:22px;padding-top:20px;border-top:1px solid #263148}.cost-card__stats{display:grid;gap:10px;margin-top:14px}.cost-card__row{display:flex;justify-content:space-between;gap:15px;font-size:12px}.cost-card__row b{color:#fff;text-align:right}.cost-card__section:first-child .cost-card__row b{color:#ff4f78}.cost-card button{width:100%;margin-top:auto;background:#e9164d}
...
table.parts-table input[data-field="length"],
table.parts-table input[data-field="width"] { display:block; width:90px; margin:0 auto; text-align:center; }
...
.source-table input[data-field="label"]{width:180px}
...
<main class="container">
    ...
    <aside class="panel cost-card" aria-label="Стоимость материала и услуг распила">
        <div class="cost-card__section">
            <div class="cost-card__heading"><div class="cost-card__eyebrow">✦ Материал</div><div class="cost-card__total" id="material-cost-total">0,00 ₽</div></div>
...
materialsList.addEventListener('focusin', event => { if (event.target.dataset.field === 'qty' && event.target.value === 'Авто') { event.target.type='number'; event.target.value=''; event.target.placeholder='Авто'; } });
...
materialsList.addEventListener('focusout', event => { if (event.target.dataset.field === 'qty' && event.target.value === '') { const material=sourceMaterials.find(m=>m.materialId===Number(event.target.closest('tr').dataset.id)); if(material) material.qty=null; event.target.type='text'; event.target.value='Авто'; event.target.classList.add('unlimited-qty'); } });
renderPanelSearch();
renderSourceMaterials();
